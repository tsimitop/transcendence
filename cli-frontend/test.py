...
def draw_paddle(window, y, x, number=0):
    for i in range(PADDLE_LENGTH):
        window.addch(int(y + i), int(x), ' ', curses.color_pair(number))
...
def main(stdscr):
    ...
    win.clear()
    win.border()
    win.addstr(int(ROWS / 2 - 2), int((COLUMNS / 2 + 1) - len('XPONG') / 2), \
    ...
    while True:        
        ...
        win.addch(int(ball_y), int(ball_x), ' ')
        ...
        if player == 11 or computer == 11:
            ...
            win.border()
            win.nodelay(1)
        win.addch(int(ball_y), int(ball_x), 'x', curses.color_pair(2))
        win.refresh()
        time.sleep(0.05)
